    ...
    return (
        <div className="space-y-6 max-w-6xl mx-auto">
            ...
            {!loading && !error && tables.length === 0 && (
                <div className="bg-[var(--bg-surface)] border border-[var(--border)] p-12 text-center text-gray-400 rounded-2xl">
            ...
            {!loading && !error && tables.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-4 shadow-xl h-[600px] flex flex-col">
                        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-widest border-b border-[var(--border)] pb-3 mb-3">
                            ...
                            {tables.map((table) => (
                                <button
                                    ...
                                    className={`w-full text-left px-3 py-2.5 rounded-xl border text-sm transition-all flex items-center justify-between ${selectedTable?.name === table.name
                                        ? "bg-[var(--accent-dim)] border-[var(--accent)] text-white font-medium shadow-md shadow-[var(--accent)]/5"
                                        : "border-transparent text-gray-400 hover:bg-[var(--accent-dim)]/50 hover:text-white"
                                        }`}
                                >
                                    ...
                                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded bg-[var(--bg-base)] text-[var(--accent-secondary)] border border-[var(--border)] whitespace-nowrap">
                    ...
                    <div className="bg-[var(--bg-surface)] border border-[var(--border)] rounded-2xl p-5 shadow-xl md:col-span-2 h-[600px] flex flex-col">
                        {selectedTable ? (
                            <div className="flex flex-col h-full space-y-4">
                                <div className="flex justify-between items-center border-b border-[var(--border)] pb-4">
                                    ...
                                    <span className="text-xs font-semibold text-gray-400 bg-[var(--bg-base)] border border-[var(--border)] px-3 py-1 rounded-lg">
                                        {selectedTable.columns.length} columns in structure
                                    </span>
                                </div>

                                <div className="flex-1 overflow-y-auto rounded-xl border border-[var(--border)] bg-[var(--bg-base)] w-full">
